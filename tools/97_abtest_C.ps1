<#
  Test C: AnotherRus catalog + OUR ru bundles + original fonts.
  Then restore vanilla afterwards is NOT done here - keep state for next test.
#>
param()
...
# apply: AnotherRus catalog + OUR bundles
...
Write-Host 'config C installed (AnotherRus catalog + our bundles)'
...
if (Test-Path $log) {
    ...
    Write-Host ("NRE count: " + (Select-String -Path $log -Pattern 'NullReferenceException').Count)
